...
void TraverseModule(void)
{
	...
	for( llvm::Module::iterator ModIter = TheModule->begin(); ModIter != TheModule->end(); ++ModIter )
	{
		...
		for( llvm::Function::iterator FuncIter = Func->begin(); FuncIter != Func->end(); ++FuncIter )
		{
			...
			std::vector< llvm::Instruction* > MulInsts;
			std::vector< llvm::Instruction* > SubInsts;
			...
			for( llvm::BasicBlock::iterator BBIter = BB->begin(); BBIter != BB->end(); ++BBIter )
			{
				...
				// if( Inst->isBinaryOp() ) {}
				if( AddInst->getOpcode() == llvm::Instruction::Add  && cnt == 0)
				{
					llvm::Instruction* MulInst = llvm::BinaryOperator::Create( 
							...
							"multmp",	/* Name */
							AddInst /* BeforeInst */ );
					AddInst->replaceAllUsesWith( MulInst );

					MulInsts.push_back( AddInst );
                    cnt++;
				}
            	else if( AddInst->getOpcode() == llvm::Instruction::Add  && cnt == 1)
				{
					...
					SubInsts.push_back( AddInst );
				}
			}

			for( int i=0, Size=MulInsts.size(); i<Size; ++i ) MulInsts[i]->eraseFromParent();
        	for( int i=0, Size=SubInsts.size(); i<Size; ++i ) SubInsts[i]->eraseFromParent();
